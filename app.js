/* Numéro affiché à côté de « Sable ». Incrémente-le à chaque déploiement,
   ...
   v3.08 — LE MOT D'ACTION DU TOAST N'ÉTAIT LISIBLE QU'EN THÈME SOMBRE (ticket #17). Dernier nœud de la passe de goût sur l'accent, et le seul que #15 avait explicitement mis de côté : le commentaire des tokens de styles.css portait « UNE exception, et elle va dans l'autre sens », concédant que `#toast .u` gardait `--accent` sur le fond sombre en dur du toast. (a) L'EXCEPTION N'EN ÉTAIT PAS UNE. Elle citait sa propre mesure — 3.71 — et s'arrêtait là. Or 3.71:1 est SOUS le seuil AA de 4.5 exigé pour du 13,5 px en graisse 600 : la note ne justifiait pas un compromis, elle enregistrait un échec en le nommant exception. Ce qu'elle démontrait vraiment, c'est qu'il ne fallait pas FONCER (`--accent-deep` clair tombe à 2.76 sur ce fond) — pas qu'il fallait s'en tenir à `--accent`. (b) LES DEUX TEXTES DU TOAST, RELEVÉS AVANT DE TRANCHER, comme le demandait le ticket. Le toast n'en a que deux, et c'est ce qui rend le relevé concluant : le message, `#fff` sur #1B1C22, à 16.99:1 — hors de cause, il n'est pas touché ; et le mot d'action, qui n'existe qu'en trois exemplaires dans tout l'app.js (« annuler » de la corbeille, « voir » du dédoublonnage, « classer » après une capture), à 3.71:1 en thème CLAIR et 7.47:1 en thème SOMBRE. Le défaut est donc à moitié invisible : un utilisateur en sombre n'a jamais rien vu, et c'est pourquoi il a survécu à #13 comme à #15. (c) CE QUI TRANCHE, ET CE N'EST PAS LE GOÛT. Le toast est la SEULE surface de l'app dont toutes les couleurs sont écrites en dur et identiques dans les deux thèmes — fond #1B1C22, bordure #2C2D36, message #fff. Un fond qui ne suit pas le thème avec un texte qui le suit : voilà l'incohérence, et le 3.71 n'en est que le symptôme. Le mot passe donc en dur comme ses trois voisines, à #D8A25A — la valeur que le thème sombre servait DÉJÀ. Conséquence : en sombre, RIEN ne change, c'est la couleur d'avant au chiffre près ; en clair, 3.71 → 7.47. `--accent-hover` clair (#95601D, 3.21) a été mesuré et écarté au passage. Aucun token créé — la valeur appartient au toast, pas à la palette, et un token de plus pour un seul mot de 7 lettres est exactement la faute écartée au ticket #16. (d) LA NOTE DES TOKENS EST RÉÉCRITE, pas seulement le sélecteur. C'était l'autre moitié du ticket : tant que le bloc `--accent-deep` annonçait une réserve, la règle « `--accent` ne peint jamais de lettres » restait une règle à trous, donc invérifiable d'un coup d'œil — ce que #15 s'était donné pour objectif. Elle n'a plus aucune exception. Vérifié : les deux contrastes recalculés sur les valeurs finales du fichier ; `var(--accent)` ne subsiste plus dans aucune règle de couleur de TEXTE de styles.css ; `#toast` n'est redéfini nulle part ailleurs qu'en position (desktop-v2.css, styles-desktop.css) — aucune surcouche ne repeint ni le fond ni le mot ; `node --check` sur app.js et sw.js. CE QUE ÇA NE RÈGLE PAS. Non vérifié au pouce : le rendu du toast en clair sur un vrai appareil, faute d'avoir pu ouvrir le harnais `.claude/` depuis cette session (le dossier est hors dépôt). La géométrie du mot ne bouge pas — même corps, même graisse, même padding — donc la zone tapable de la v3.03 est inchangée. Restent ouverts, inchangés, les mêmes qu'en v3.07, et côté suivi les tickets #12, #18, #19 et #22 de la passe de goût. À remplacer : app.js, styles.css et sw.js, cache v105 -> v106. index.html n'est PAS touché. */
