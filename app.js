...
function maybeOpenFrame(){
  ...
  if(new Date().getHours()<frameHour())return;   /* pas avant l'heure dite */
  if(layers.length)return;                       /* une couche ouverte : on ne se met pas en travers */
  ensureBatch();
...
setInterval(()=>{if(uiReady)maybeOpenFrame();},60000);
...
/* v2.98 — le pas à pas saisissable. La grille de vingt-quatre pastilles a
   ...
   feuille. Trois cotes ici, et une seule qui compte. (a) Le champ est un
   `contenteditable="plaintext-only"`, JAMAIS un `input` : l'auto-remplissage
   de Chrome ne s'accroche qu'aux `input`/`textarea`/`select`, donc pas de
   barre, donc pas de vue qui respire — c'est la sortie annoncée en v2.94 et
   repoussée trois fois, essayée ici sur deux chiffres avant la feuille de
   capture. (b) Seules les heures se saisissent ; « :00 » est du décor et ne
   reçoit jamais le curseur, ce qui tient la promesse « pas de minutes » sans
   avoir à l'écrire dans une aide. (c) Un chiffre >= 3 vaut pour lui-même : il
   ne peut plus être une dizaine d'heures, l'attendre coûterait un appui sur
   deux. C'est la règle des sélecteurs d'heure natifs. */
function setHours(){
  const id=_setId("sh");
  _setWire.push(()=>{
    const el=document.getElementById(id); if(!el)return;
    const box=el.querySelector(".stpv"), hh=el.querySelector(".stpn");
    if(!box||!hh)return;
    /* `plaintext-only` peut ne pas être reconnu : on REGARDE ce que le moteur a
       retenu au lieu de le supposer, et on retombe sur `true` — le champ ne fait
       que deux chiffres et `input` les filtre déjà, le repli est sans danger. */
    hh.setAttribute("contenteditable","plaintext-only");
    if(hh.contentEditable!=="plaintext-only")hh.setAttribute("contenteditable","true");
    let live=false, prev=frameHour();
    const p2=h=>(h<10?"0":"")+h;
    const paint=h=>{hh.textContent=p2(h);};
    const put=h=>{
      h=Math.max(0,Math.min(23,h|0));
      settings.frameHour=h;saveSettings();paint(h);
      /* Reposer une heure DÉJÀ passée doit pouvoir servir le jour même : la
         relecture de l'heure s'en charge à la minute qui suit, une fois cette
         feuille refermée (`maybeOpenFrame` s'abstient sous une couche). */
    };
    const selAll=()=>{try{const r=document.createRange();r.selectNodeContents(hh);
      const s=getSelection();s.removeAllRanges();s.addRange(r);}catch(e){}};
    const caretEnd=()=>{try{const r=document.createRange();r.selectNodeContents(hh);r.collapse(false);
      const s=getSelection();s.removeAllRanges();s.addRange(r);}catch(e){}};

    hh.addEventListener("focus",()=>{prev=frameHour();live=true;box.classList.add("on");selAll();});
    hh.addEventListener("blur",()=>{
      live=false;box.classList.remove("on");
      const t=hh.textContent.replace(/\D/g,"");
      /* Un champ vidé n'est pas une demande de minuit : on rend la valeur d'avant. */
      if(!t)paint(frameHour()); else{put(+t);haptic(8);}
    });
    hh.addEventListener("beforeinput",e=>{
      if(e.inputType==="insertLineBreak"||e.inputType==="insertParagraph"){e.preventDefault();hh.blur();return;}
      if(e.inputType&&e.inputType.indexOf("insert")===0&&!/^\d*$/.test(e.data||""))e.preventDefault();
    });
    hh.addEventListener("input",()=>{
      const raw=hh.textContent, t=raw.replace(/\D/g,"").slice(0,2);
      if(t!==raw){hh.textContent=t;caretEnd();}
      if(t.length===2||(t.length===1&&+t>=3))hh.blur();
    });
    hh.addEventListener("keydown",e=>{
      if(e.key==="Escape"){e.preventDefault();paint(prev);put(prev);hh.blur();}
    });

    /* Les flèches restent pour le voisin immédiat : lever le clavier pour passer
       de 7 h à 8 h serait plus cher que le pas qu'on économise. Le bouclage
       23 -> 0 est celui d'une horloge, pas une butée oubliée. */
    el.querySelectorAll("button[data-d]").forEach(b=>{
      const d=+b.dataset.d;
      const step=()=>{if(live)return;put((frameHour()+d+24)%24);haptic(8);};
  ...
  return `<div class="stp" id="${id}">`
    +`<button class="stpb" data-d="-1" aria-label="Heure précédente">−</button>`
    +`<span class="stpv"><span class="stpn" inputmode="numeric" enterkeyhint="done" role="textbox"`
    +` aria-label="Heure d’arrivée, de 0 à 23" spellcheck="false" autocapitalize="off">`
    +`${(cur<10?"0":"")+cur}</span><span class="stpm">:00</span></span>`
    +`<button class="stpb" data-d="1" aria-label="Heure suivante">+</button>`
    +`</div>`;
}
function setMutes(){
  const id=_setId("sm");
