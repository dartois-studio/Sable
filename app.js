...
(function(){
  ...
  addEventListener("resize",()=>{if(!document.body.classList.contains("scoped"))paintTabs(curTab,0,false);});
...
function openGotoSheet(){
  ...
  list.querySelectorAll("[data-gocat]").forEach(x=>x.onclick=()=>{const n=x.dataset.gocat;closeSheet(true);enterCollection(n);});
...
/* v2.60 — « Aller à » un palier ne défilait pas dans Ma pile. Le saut passait par
   ...
   au vrai défileur (le document). Le saut d'un palier vers une CATÉGORIE marchait
   parce qu'il ne défile pas — il appelle enterCollection ; seul le palier de date,
   qui a besoin de défiler, tombait à plat, et aucun banc ne pouvait le voir.
   On vise désormais le bon défileur à la main : le document hors surface, #tab-pile
   en surface de périmètre (fixed, overflow-y:auto). L'en-tête collant est mesuré au
   moment du saut pour poser le palier juste dessous (le `scroll-margin-top:64px` du
   CSS ne valait que pour scrollIntoView, qu'on n'appelle plus). Ce n'est PAS la faute
   --tbh : lire une position pour DÉFILER est le métier du JS ; l'interdit ne porte
   que sur une mesure JS qui nourrit un positionnement CSS. */
function jumpToAnchor(id){
  const el=document.getElementById(id);if(!el)return;
  const scoped=document.body.classList.contains("scoped");
  const head=scoped?document.getElementById("scopeHead"):document.querySelector(".topbar");
  const off=((head&&head.getBoundingClientRect().height)||0)+6;   /* sous l'en-tête collant */
  const r=el.getBoundingClientRect();
  if(scoped){
    const sc=document.getElementById("tab-pile");if(!sc)return;
    const top=r.top-sc.getBoundingClientRect().top+sc.scrollTop-off;
    sc.scrollTo({top:Math.max(0,top),behavior:"smooth"});
  }else{
    const top=r.top+window.scrollY-off;
