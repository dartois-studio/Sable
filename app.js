...
function maybeOpenFrame(){
  ...
  frameTucked=false;settings.frameDay=todayStr();saveSettings();
  renderRiseFrame();
  const b=document.getElementById("inboxBtn");
...
function closeSheet(skipSave){
  ...
  setTimeout(()=>{
    ...
    sh.classList.remove("tall","fiche");  /* v2.71 — l'encre et la gouttière d'une fiche ne survivent pas à la feuille */
...
/* ────────────────────────────────────────────────────────────────────────────
   v2.92 — SONDE DE MESURE, TEMPORAIRE. Trois correctifs, trois échecs : le saut
   de 132 px de la feuille d'ajout ne vient d'aucune des trois causes que j'ai
   avancées. Je cesse de raisonner à la place de l'appareil. La sonde vit HORS de
   la feuille (`position:fixed`, par-dessus la zone voilée) pour ne rien ajouter
   à la hauteur qu'on cherche justement à mesurer, et elle relève à chaque image
   les EXTRÊMES — le saut ne dure qu'une image, un affichage instantané ne le
   montrerait jamais. Une capture d'écran suffira à trancher :
     vue   = innerHeight (hauteur de mise en page)
     vis   = visualViewport.height et son décalage haut
     max   = la max-height CALCULÉE de la feuille (dit aussi si styles.css v2.91
             est bien servi : on doit y lire ~93 % de « vue », pas 88 %)
     feuil = hauteur réelle de la feuille, et son bord haut
     bas   = safe-area-inset-bottom, lue sur une éprouvette
   Chaque ligne donne « min→max » : si une valeur ne bouge pas, elle est
   innocente ; celle qui bouge de 132 px (à ce facteur d'échelle : 48 px CSS)
   est la coupable, et il n'y en aura qu'une.
   ──────────────────────────────────────────────────────────────────────────── */
function capDiag(){
  document.getElementById("capDiagBox")?.remove();
  const box=document.createElement("div");
  box.id="capDiagBox"; box.className="capdiag";
  const probe=document.createElement("i");     /* éprouvette pour lire env() */
  probe.style.cssText="position:fixed;left:-9999px;height:env(safe-area-inset-bottom)";
  document.body.appendChild(probe); document.body.appendChild(box);
  const sh=document.getElementById("appSheet");
  const ext={};
  const note=(k,v)=>{v=Math.round(v); const e=ext[k]; if(!e)ext[k]=[v,v];
    else{if(v<e[0])e[0]=v; if(v>e[1])e[1]=v;}};
  const fmt=k=>{const e=ext[k]; if(!e)return "?"; return e[0]===e[1]?""+e[0]:e[0]+"→"+e[1];};
  const tick=()=>{
    if(!box.isConnected||!sh.classList.contains("open")){probe.remove();box.remove();return;}
    const vv=window.visualViewport;
    note("vue",innerHeight);
    if(vv){note("vis",vv.height);note("vtop",vv.offsetTop);}
    note("max",parseFloat(getComputedStyle(sh).maxHeight)||0);
    note("feuil",sh.offsetHeight);
    note("haut",sh.getBoundingClientRect().top);
    note("bas",probe.getBoundingClientRect().height);
    box.textContent="vue "+fmt("vue")+" · vis "+fmt("vis")+"/"+fmt("vtop")
      +" · max "+fmt("max")+" · feuil "+fmt("feuil")+" haut "+fmt("haut")
      +" · bas "+fmt("bas")+" · "+APP_VERSION;
...
function openCaptureSheet(){
  ...
  capDiag();   /* v2.92 — SONDE TEMPORAIRE. Se retire à la version suivante. */
