...
let frameTucked=false, frameH=0, frameIO=null, frameAuto=false, frameAnim=false;
...
(function(){
  ...
  const ready=()=>{
    if(!frameOn||!frameTucked||frameAuto)return null;
...
function toggleRiseFrame(){
  /* Rangé par le défilement, l'enveloppe le fait RESSORTIR — elle ne le ferme
     pas une seconde fois. Trois états à l'écran, deux seulement à décider. */
  if(frameOn&&frameTucked){
    if(settings.frameDay!==todayStr()){settings.frameDay=todayStr();saveSettings();}
    frameScrollTop(()=>untuckFrame());
    return;
  }
  frameTucked=false;
  frameOn=!frameOn;
...
function maybeOpenFrame(){
  ...
  frameOn=true;frameTucked=false;settings.frameDay=todayStr();saveSettings();
  renderRiseFrame();
  const b=document.getElementById("inboxBtn");
