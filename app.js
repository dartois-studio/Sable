...
/* v2.97 — les vingt-quatre heures. On EMPRUNTE `.seg` au lieu d'inventer un
   second composant de choix : c'est une grille de colonnes `1fr`, six par
   rangée la replient d'elles-mêmes en quatre lignes. Pas d'`input[type=time]`
   — le fichier traîne déjà « Une date précise » en natif comme une dette, et
   les minutes n'ont aucun sens pour un seuil qu'on franchit une fois par jour. */
function setHours(){
  const id=_setId("sh");
  _setWire.push(()=>{
    const el=document.getElementById(id); if(!el)return;
    el.querySelectorAll("button").forEach(b=>b.onclick=()=>{
      el.querySelectorAll("button").forEach(x=>x.classList.remove("on"));
      b.classList.add("on");haptic(8);
      settings.frameHour=+b.dataset.h;saveSettings();
  ...
  const cur=frameHour();
  let out=`<div class="seg hrs" style="--n:6" id="${id}">`;
  for(let h=0;h<24;h++)out+=`<button data-h="${h}" class="${h===cur?"on":""}">${h}h</button>`;
  return out+`</div>`;
}
function setMutes(){
  const id=_setId("sm");
...
function openSettingsSheet(){
  ...
  if(surfaceOn()){
    ...
    surf+=setStack("Heure d’arrivée","Le cadre attend cette heure. Avant, la journée n’est pas dépensée.",setHours());
  }
