/* Numéro affiché à côté de « Sable ». Incrémente-le à chaque déploiement,
   ...
   v3.28 — LE QUADRILLAGE NE DIT PLUS DEUX CHOSES : COLLECTION PREND LE DOSSIER (ticket #65). LE DÉFAUT, ouvert par le #61 et assumé à l'ouverture : le glyphe retenu pour la forme « Cartes » est GÉOMÉTRIQUEMENT IDENTIQUE à `icons.svg#grid` — quatre carrés 7×7, mêmes coordonnées, même arrondi — et `#grid` était déjà l'onglet Collection de la barre du bas. Le même dessin disait donc, dans le MÊME écran, « affiche l'index en cartes » en haut et « tu es sur Collection » en bas. Trois issues étaient ouvertes ; c'est la (c) qui est prise, sur capture (mockup/ticket-65-glyphe/capture.html, les deux emplois entourés d'après leurs rectangles mesurés) : ON CHANGE L'ONGLET, PAS LA FORME. La raison n'est pas d'arbitrage mais de sens — « Collection » est un LIEU, et un dossier le dit ; un quadrillage décrit une mise en page, ce qui est le travail de l'autre glyphe. AUCUN SYMBOLE NOUVEAU : `#folder` est dans le sprite depuis longtemps, et c'est DÉJÀ le glyphe de « Catégorie » dans la fiche (#rowCat) — le changement ne crée pas une convention, il applique celle qui existait à moitié. QUATRE ENDROITS, et il fallait les quatre : l'onglet Collection (index.html), le bouton « Catégorie » de la barre de lot (index.html — sans lui, `#grid` aurait continué de dire « catégorie » quelque part, doublon simplement déplacé), le glyphe des entrées de CATÉGORIE dans les résultats de recherche (app.js, la constante s'appelait GRID_IC et devient CAT_IC : elle rendait `grid` à côté de noms de catégories, c'est-à-dire le glyphe d'une forme d'affichage), et TAB_ICON de la liste réordonnable des Réglages (app.js — cette liste EST la barre du bas, deux dessins pour le même onglet selon l'endroit aurait été pire que le doublon défait). CE QUI NE BOUGE PAS, et c'est le point du ticket : la forme « Cartes » GARDE `#grid`. C'est le dessin juste pour l'objet, et c'est celui qui a été demandé. Ce que `#grid` signifie devient donc unique et vrai partout : une mise en page en grille — la forme « Cartes » de l'index, et la forme « Grille » de Ma pile. VÉRIFIÉ, en balayant les sept occurrences restantes de la chaîne "grid" : cinq sont des VALEURS stockées (la forme « Grille » de Ma pile, et l'ancienne valeur d'indexView que loadSettings migre), une est le glyphe de « Cartes » qui reste par décision — et la septième est le repli `TAB_ICON[n]||"grid"` de setTabOrder, LAISSÉ TEL QUEL et signalé plutôt que corrigé : il est mort (tabOrder() ne rend jamais autre chose qu'une permutation complète de TAB_ORDER, établi en v3.10), et lui inventer une valeur aurait été un choix arbitraire dans un ticket qui n'en demande pas. Aussi vérifié : `#folder` existait déjà dans icons.svg (aucune ligne ajoutée au sprite) ; node --check sur app.js. NON VÉRIFIÉ : le pouce — un repère de navigation permanent qui change de dessin se paie en désorientation les premiers jours, et aucune capture ne dit ça. À remplacer : index.html, app.js, sw.js. Cache v125 -> v126.
*/
...
/* Ticket #36 — LE JETON DE LA LIGNE LUE. `updated_at` de `brain:v1:items` au
   moment de la dernière lecture ou écriture CONFIRMÉE : ce que la base disait la
   dernière fois qu'on en était sûr. Personne ne le compare encore — l'étape 3 du
   ticket le fera relire par `_writeItems` avant d'écrire, et un jeton qui a
   bougé signifiera qu'un autre appareil est passé. Deux règles pour qui reprend
   ce travail : (a) il se pose depuis une lecture RÉUSSIE, jamais depuis une
   valeur devinée — même logique que `_lastN` (v3.23) ; (b) toute écriture
   réussie devra le RAFRAÎCHIR, sinon la deuxième écriture d'affilée du même
   appareil se prendrait pour un conflit avec la première, et le ticket exige que
   le cas normal reste silencieux. `null` est légitime : ligne absente (compte
   neuf) ou lecture non confirmée. */
let _kvToken=null;
async function loadState(){
  let raw=null;
  try{const r=await window.storage.get(KEY_ITEMS); raw=r&&r.value?r.value:null; _kvToken=r?(r.updated_at||null):null;}
  catch(e){console.error("[loadState] lecture",e);stateReady=false;_kvToken=null;return false;}
...
/* Ticket #36 — DEUX APPAREILS QUI ÉCRIVENT. `kv` n'a ni version ni comparaison :
   le dernier `upsert` gagnait, EN SILENCE, et les gestes de l'autre appareil
   partaient sans que personne l'apprenne jamais. On relit le jeton (`updated_at`,
   étapes 1 et 2) avant d'écrire : s'il a bougé, quelqu'un d'autre est passé.

   QUATRE CHOSES QUI ONT ÉTÉ TRANCHÉES, et qui expliquent la forme du code.

   (1) OÙ. Dans `_writeItems` et nulle part ailleurs, pour la raison de la v2.88 :
   les appels absorbés par `_wrPend` n'arrivent JAMAIS ici — seule l'écriture qui
   PART y passe. Le piège annoncé par le ticket (« le jeton doit être relu par
   l'écriture qui part, pas par celle qui a été absorbée ») est donc réglé par le
   placement, pas par du code de coalescence.

   (2) LE COÛT, ET LA FENÊTRE AVEUGLE. Relire avant CHAQUE geste, c'est un
   aller-retour de plus à chaque geste, sur le lien du téléphone que la v2.88 a
   passé du temps à désencombrer. On ne relit donc que si la dernière écriture
   RÉUSSIE date de plus de `CONFLIT_GRACE_MS` : deux gestes d'affilée sur le même
   appareil ne peuvent pas se concurrencer eux-mêmes. Le prix est explicite —
   pendant ces 20 secondes on n'a pas d'yeux, et une écriture d'un autre appareil
   qui tombe pile là sera écrasée. C'est un compromis assumé, pas un oubli.

   (3) LA RELECTURE QUI RATE N'EMPÊCHE PAS D'ÉCRIRE. Un réseau qui vacille ne doit
   pas devenir « impossible d'enregistrer » : le détecteur échoue OUVERT. Si
   l'écriture elle-même échoue, la chaîne de la v2.66 le dira — c'est son travail,
   pas celui d'ici.

   (4) LE CAS NORMAL RESTE SILENCIEUX, exigence explicite du ticket. Un appareil
   seul ne voit jamais son propre jeton bouger, parce que `storage.set` rend
   l'horodatage qu'il vient d'écrire et que `_kvToken` le prend. Sans ça la
   DEUXIÈME écriture d'affilée se prendrait pour un conflit avec la première.

   CE QUE ÇA NE FAIT PAS : fusionner. Il faudrait un horodatage par item, donc un
   champ, donc une migration. On prévient et on propose de recharger — ce qui perd
   le geste en cours, et le `confirm()` le dit. Écraser reste possible, mais plus
   jamais sans que personne l'ait su.

   OPTION (a) ASSUMÉE (décision du 07/09/2026) : `updated_at` est écrit par le
   CLIENT, pas par Postgres. Le jeton ne vaut donc que ce que valent les horloges
   des appareils, et deux écritures dans la même milliseconde rendent un jeton
   identique. On l'accepte : un aller-retour réseau sépare deux écritures réelles.
   Le durcissement, si un jour il le faut, est côté base — `DEFAULT now()` plus un
   trigger `BEFORE UPDATE`, et `storage.set` cesse d'envoyer la valeur. */
const CONFLIT_GRACE_MS=20000;
/* Ticket #60 — LE JETON SE COMPARE EN INSTANTS, PLUS EN TEXTE. Le défaut que
   la v3.25 a livré : les deux jetons comparés ne viennent PAS de la même plume.
   Celui d'une LECTURE est sérialisé par PostgREST depuis la colonne
   `timestamptz` — « 2026-09-07T09:12:33.412+00:00 ». Celui d'une ÉCRITURE est la
   chaîne que `storage.set` a fabriquée lui-même avec `toISOString()` — « …412Z »
   pour le MÊME instant. Un `!==` sur ces deux textes est vrai à tous les coups :
   dès qu'une écriture a réussi dans la session, la suivante qui sort de la
   fenêtre de grâce se déclare en conflit avec elle-même.
   POURQUOI LES BANCS NE L'ONT PAS VU : le shim de `.claude/dev-harness.js` et
   celui du banc rendent la chaîne qu'on leur a donnée, à l'octet — ils n'ont pas
   de Postgres au milieu, donc pas de resérialisation. Le contrôle au pouce ne
   l'a pas vu non plus, et pour une raison exacte : « enchaîner cinq
   enregistrements » les garde TOUS dans les vingt secondes de grâce, où l'on ne
   relit rien. Il fallait une écriture, PUIS une pause de plus de 20 s, PUIS une
   écriture — c'est-à-dire précisément le parcours du partage : l'item est écrit
   par la capture optimiste, la fiche s'ouvre, on la remplit, on enregistre.
   LA FORME DU CORRECTIF. On ne compare plus des textes mais l'INSTANT qu'ils
   désignent, `Date.parse` des deux côtés. Aucune tolérance n'est ajoutée : deux
   écritures réelles sont séparées par un aller-retour réseau, donc par des
   millisecondes différentes — la limite « même milliseconde = conflit invisible »
   de la v3.25 est inchangée, ni aggravée ni masquée. Un jeton illisible fait
   échouer OUVERT, comme la relecture ratée (décision (c) de la v3.25).
   CE QUI RESTE VRAI : le jeton est toujours écrit par le client (option (a) du
   07/09). Si un jour la base le prend en charge — `DEFAULT now()` + trigger — la
   valeur devinée par `storage.set` cesserait d'être celle qui est stockée, et la
   normalisation ne suffirait plus : il faudrait relire l'horodatage écrit, par
   `.upsert(...).select('updated_at')`. C'est la forme à prendre à ce moment-là,
   pas avant. */
function _jetonMs(ts){
  if(!ts)return null;
  const t=typeof ts==="number"?ts:Date.parse(ts);
  return Number.isFinite(t)?t:null;
}
let _lastWriteAt=0,_conflitVu=false;
async function _autreAppareilEstPasse(){
  if(_kvToken===null)return false;                                  /* rien à comparer */
  if(Date.now()-_lastWriteAt<CONFLIT_GRACE_MS)return false;         /* on vient d'écrire */
  const local=_jetonMs(_kvToken);
  if(local===null)return false;                                     /* jeton illisible : échouer ouvert */
  let r=null;
  try{r=await window.storage.get(KEY_ITEMS);}
  catch(e){console.warn("[saveItems] jeton non relu, on écrit quand même",e);return false;}
  const distant=_jetonMs(r&&r.updated_at);
  if(distant===null)return false;
  return distant!==local;
}
async function _writeItems(){
  SAVE_FAIL_MSG=SAVE_FAIL_RESEAU;
  ...
  _wipeOk=false;
  if(await _autreAppareilEstPasse()){
    SAVE_FAIL_MSG=SAVE_FAIL_CONFLIT;
    console.error("[saveItems] refusé : le jeton a bougé — un autre appareil a écrit depuis le chargement");
    /* On ne demande QU'UNE FOIS. Refuser puis reposer la question à chaque
       enregistrement suivant transformerait la garde en harcèlement modal ; le
       refus, lui, ne s'épuise pas — tant que l'écran n'a pas rechargé, aucune
       écriture ne part. Le toast de la v2.66 dit pourquoi à chaque fois. */
    if(!_conflitVu){
      _conflitVu=true;
      if(confirm("Un autre appareil a modifié ta pile depuis que cet écran l'a chargée.\n\n"
        +"Enregistrer maintenant écraserait ses modifications.\n\n"
        +"OK — recharger : tu récupères la version à jour, et le geste en cours est perdu.\n"
        +"Annuler : rien n'est enregistré, l'écran reste tel quel.")){
        location.reload();
      }
    }
    return false;
  ...
  try{
    const w=await window.storage.set(KEY_ITEMS,JSON.stringify(items));
    _lastN=items.length;
    /* Le jeton de CE qu'on vient d'écrire, rendu par la couche de stockage : pas
       de relecture, donc pas d'aller-retour de plus. Absent (vieille couche) =>
       null, ce qui désarme la comparaison au lieu de la fonder sur un jeton
       périmé — échouer ouvert, comme la relecture. */
    _kvToken=(w&&w.updated_at)||null;
    _lastWriteAt=Date.now();
    saveMirror();return true;
  }catch(e){console.error("[saveItems]",e);return false;}
}
/* Ticket #36 — `SAVE_FAIL_MSG` n'est plus une constante : c'est le DERNIER MOTIF
   D'ÉCHEC CONNU, posé par `_writeItems` et lu tel quel par les neuf sites qui
   toastent. Sans ça un refus de conflit s'annoncerait « réseau ou session »,
   c'est-à-dire un diagnostic faux sur le seul écran où l'on décide quoi faire. */
const SAVE_FAIL_RESEAU="Pas enregistré — réseau ou session.";
const SAVE_FAIL_CONFLIT="Pas enregistré — un autre appareil a modifié la pile. Recharge.";
let SAVE_FAIL_MSG=SAVE_FAIL_RESEAU;
...
/* Ticket #57 — CACHER N'EST PAS RETIRER. La v3.17 pariait que fermer la fiche
   arrêtait la lecture, « comportement natif quand le nœud est retiré ». La
   prémisse était fausse : personne ne retire rien. `closeSheet` ôte une classe
   et le CSS cache la feuille — #sheetList n'est réécrit qu'à la PROCHAINE
   ouverture ; `closeRemontee` pose `hidden` sur #rise et s'en tient là. Le
   lecteur reste dans le document, invisible, et continue de jouer.
   C'est la même famille que l'invariant « une règle display: sur une cible
   masquable réclame son annulation » : un média ne se soucie pas d'être vu.
   On met en PAUSE, on ne vide pas le conteneur — les feuilles s'enchaînent
   (garde v2.89), et le vider ferait clignoter l'enchaînement. */
function stopMedia(root){
  if(!root)return;
  root.querySelectorAll("video,audio").forEach(m=>{try{m.pause();}catch(e){}});
}
...
function closeRemontee(){
  ...
  const el=document.getElementById("rise");if(!el)return;
  el.hidden=true;
  /* Ticket #57 — même défaut que la fiche, trouvé en livrant : `hidden` cache
     la surface mais laisse la carte et son lecteur dans le document. */
  stopMedia(el);
...
function renderRootSearch(){
  ...
  /* Ticket #65 — le glyphe d'une CATÉGORIE dans les résultats, pas celui d'une
     forme d'affichage : `folder` comme l'onglet Collection et comme #rowCat.
     Il s'appelait GRID_IC et rendait `grid`, qui depuis le #61 dit « cartes ». */
  const CAPC=6,CAPT=8,CAT_IC=icon('folder');
  ...
  const entCat=(name,f,n)=>`<button class="ent" data-cat="${esc(f)}"><span class="eic">${CAT_IC}</span><span class="enm">${hlMatch(name,q)}</span><span class="ecnt">${n}</span>${CHEV_R}</button>`;
...
function closeSheet(skipSave){
  ...
  /* Ticket #57 — tout de suite, et non dans le setTimeout de 300 ms plus bas :
     personne ne doit entendre trois dixièmes de son de plus. */
  stopMedia(document.getElementById("sheetList"));
...
/* Ticket #65 — `categories` suit l'onglet : dossier et non quadrillage. Cette
   liste EST la barre du bas, réordonnable ; deux dessins pour le même onglet
   selon l'endroit serait pire que le doublon qu'on vient de défaire. */
const TAB_ICON={rise:"resurface",categories:"folder",pile:"pile"};
